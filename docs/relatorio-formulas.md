...
Testes ao final da branch (Fases 1–4): 89 arquivos unitários (14 novos), 951 casos verdes (102 novos), 3 pulados por dependerem de Redis (`npm test`); E2E Playwright em modo demo com o spec novo `e2e/dashboard-painel-periodo.spec.ts` verde; `npm run typecheck` limpo. Só da auditoria (Fase 1): 11 arquivos e 72 casos novos.
...
Arquivos: `src/core/period-panel.ts` (cálculo puro), `src/skills/relatorios/index.ts` (ação `period_panel`), `src/adapters/prisma/repos.ts` e `src/adapters/memory/repos.ts` (agregação no banco), `src/app/api/_lib/dashboard.ts` + `src/app/api/v1/dashboard/period-panel/route.ts` (endpoint), `src/app/(app)/_lib/period-panel.tsx` (painel), `_lib/period-panel-query.ts` (filtros ↔ URL), `_lib/charts.tsx:LabeledBarChart`.

Regime de CAIXA (data do pagamento/recebimento efetivo, nunca o vencimento); período inclusivo nas duas pontas, no fuso da empresa; cancelados e estornados fora; agregação feita no banco (`GROUP BY`), nunca somando títulos no navegador.

Conferência com dados reais (cópia de 17/09/2026), dois meses fechados, endpoint × SQL independente:

| Mês | Total Recebido | Total Pago | Custo Fixo | Custo Variável | Não classificado | Sem centro | Resultado |
|---|---|---|---|---|---|---|---|
| jul/2026 | R$ 245.473,59 (20) × 24547359 | R$ 244.707,48 (79) × 24470748 | R$ 140.414,25 × 14041425 | R$ 104.293,23 × 10429323 | R$ 0,00 × 0 | R$ 129.387,64 × 12938764 | OK |
| ago/2026 | R$ 226.193,37 (23) × 22619337 | R$ 230.280,24 (65) × 23028024 | R$ 109.009,29 × 10900929 | R$ 121.270,95 × 12127095 | R$ 0,00 × 0 | R$ 0,00 × 0 | OK |

Agosto por centro (endpoint × SQL): Rateio Geral 10496551, Diretoria 8670026, Engenharia 2188860, Administrativo 1262144 — iguais. Ranking de categorias de agosto: Transferencia Entre Contas R$ 47.000,00 (20,41%), Antecipação De Dividendos R$ 37.205,39 (16,16%), Impostos E Taxas Diversos R$ 36.577,75 (15,88%) — iguais ao SQL. Baixas sem Payment: 0 (aviso não exibido).

| ID | Tela/Componente | Indicador | Fórmula | Origem dos dados | Filtros e regras | Arquivo:função | Teste | Tela × recalculado | Resultado | Observações |
|---|---|---|---|---|---|---|---|---|---|---|
| PNL-01 | Filtros Ano/Mês/Dia inicial/Dia final | Período | Padrão: ano e mês correntes, dia 1..último; `mes=todos` = ano inteiro (dias ignorados/desabilitados); dias > tamanho do mês são ajustados; dia inicial > final = erro (na UI, ajuste automático). Estado na query string (`ano`, `mes`, `de`, `ate`, `cc`, `cat`) | — | inclusivo | core/period-panel.ts:resolvePeriod, defaultPeriodInput; _lib/period-panel-query.ts:parsePanelQuery, panelQueryToSearch; api/_lib/dashboard.ts:toPeriodPanelInput | period-panel.test.ts (core, 6 casos); dashboard.test.ts (api, 5); relatorios/period-panel.test.ts::"sub-período de dias…", ::"mês 'Todos'…", ::"validações…"; e2e/dashboard-painel-periodo.spec.ts | URL `?ano=2026&mes=8&de=1&ate=31` → "01/08/2026 a 31/08/2026" | OK | Ano: lista dos anos com pagamento executado ou recebimento (∪ ano corrente). |
| PNL-02 | Caixa "Total Recebido no período" | Σ recebido | `SELECT SUM(amountCents), COUNT(*) FROM Receipt WHERE companyId=? AND status <> 'canceled' AND receivedDate BETWEEN :de AND :ate` | Receipt.amountCents, .receivedDate, .status | Estornados fora; valor efetivamente recebido (inclui multa/juros) — decisão 2 | prisma/repos.ts:receipts.sumRegisteredBetween (aggregate); core/period-panel.ts; relatorios:executePeriodPanel | relatorios/period-panel.test.ts::"agosto/2026: recebido, pago…" (estornado fora, encargos contam, 01/09 fora); e2e | ago R$ 226.193,37 × 22619337; jul 24547359 | OK | |
| PNL-03 | Caixa "Total Pago no período" | Σ pago | `SELECT ..., SUM(y.amountCents), COUNT(*) FROM Payment y JOIN Payable p ON p.id=y.payableId WHERE y.companyId=? AND y.status='executed' AND y.executedAt IS NOT NULL AND p.status <> 'canceled' AND ((y.executedAt AT TIME ZONE 'UTC') AT TIME ZONE :tz)::date BETWEEN :de AND :ate GROUP BY p.costClassification, p.costCenterId, p.supplierCategory` | Payment.amountCents, .executedAt, .status; Payable.status/.costClassification/.costCenterId/.supplierCategory | Só executados; título cancelado fora; parcial conta pelo valor pago; data no fuso da empresa (timestamp UTC → SP) | prisma/repos.ts:payments.sumExecutedByDimensions; core/period-panel.ts:summarizePaid | relatorios/period-panel.test.ts (22h30 de 31/08 = agosto; cancelado com pagamento fora; aprovado não executado fora; parcial 700) | ago R$ 230.280,24 (65) × 23028024; jul 24470748 (79) | OK | Transferências/dividendos lançados como títulos entram (D2, declarado no rodapé). |
| PNL-04 | Caixa "Custo Fixo pago" | parcela fixed | Σ dos grupos com costClassification = 'fixed' | idem PNL-03 | | core/period-panel.ts:summarizePaid | core/period-panel.test.ts; relatorios/period-panel.test.ts | ago R$ 109.009,29 × 10900929 | OK | |
| PNL-05 | Caixa "Custo Variável pago" | parcela variable | idem 'variable' | idem | | idem | idem | ago R$ 121.270,95 × 12127095 | OK | |
| PNL-06 | Linha discreta "Não classificado" (no card Pago) | Total Pago − Fixo − Variável | identidade Fixo + Variável + Não classificado = Total Pago | idem | aparece só quando > 0 | core/period-panel.ts | core/period-panel.test.ts::"totais e identidade…"; relatorios/period-panel.test.ts | 0 nos dois meses (100% classificado) | OK | |
| PNL-07 | Caixa "Total Gasto no Centro de Custo" + select | Σ do centro | parcela do Total Pago com costCenterId = centro; "Todos" = Total Pago; faixa "Sem centro"; identidade Σ centros + Sem centro = Total Pago | idem + CostCenter.code/.name/.active/.scope | Select: centros ativos com destino payable/both, "código — nome" | core/period-panel.ts:summarizePaid; relatorios:executePeriodPanel | core/period-panel.test.ts::"centros com movimento…"; relatorios/period-panel.test.ts::"centro de custo e categoria…"; e2e | ago: Rateio Geral 10496551 etc. = SQL | OK | Centro inexistente na query → 400. |
| PNL-08 | Caixa "Total Gasto por Categoria" + select em cascata + ranking | Σ da categoria no centro | parcela do total do centro com supplierCategory = categoria; select lista só categorias com movimento no centro (ou em todos); com "Todos": ranking (valor e % do centro, 2 casas); identidade Σ categorias = total do centro | Payable.supplierCategory (Categoria de Fornecedores do título) | Sem vínculo cadastral novo; "(sem categoria)" quando vazio | idem | core/period-panel.test.ts::"centro selecionado: … cascata…", ::"percentual … 2 casas"; relatorios/period-panel.test.ts; e2e (cascata + URL) | ago ranking = SQL (20,41 / 16,16 / 15,88%) | OK | |
| PNL-09 | Gráfico de 4 barras | Recebido, Pago, Fixo, Variável | mesmos valores de PNL-02..05; cores --ok/--crit/--brand/--warn; rótulo em R$ pt-BR e tooltip; reage SÓ ao período; estado vazio "Sem movimentação no período selecionado" quando os 4 são zero | idem | | _lib/charts.tsx:LabeledBarChart; _lib/period-panel.tsx | render.test.ts (charts); e2e (rótulo da barra "Pago", estado vazio) | rótulos iguais aos cards | OK | |
| PNL-10 | Aviso "N título(s) baixado(s) pela conciliação sem registro de pagamento" | contagem | `COUNT(*) FROM Payable p WHERE paidCents > 0 AND paidCents > COALESCE((SELECT SUM(amountCents) FROM Payment WHERE payableId=p.id AND status='executed'),0)` | Payable.paidCents; Payment | Ignorados no painel (decisão 1); alerta info no rodapé | prisma/repos.ts:payables.countSettledWithoutPayment; relatorios:executePeriodPanel | relatorios/period-panel.test.ts::"aviso de títulos baixados sem Payment…" | 0 em produção | OK | |
| PNL-11 | Rodapé Fontes/Suposições | envelope | data_sources = payments, payables, receipts, cost_centers; suposições: regime de caixa, cancelados fora, parcial pelo pago, não classificado/sem centro, transferências incluídas (D2), fonte = títulos liquidados, dias ajustados (quando houver); confiança 1,0 | — | | relatorios:executePeriodPanel; _lib/result-meta.tsx | relatorios/period-panel.test.ts; e2e | visível | OK | |
