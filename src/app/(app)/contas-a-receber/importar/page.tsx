...
/**
 * Importação de títulos a receber por CSV, em duas etapas explícitas:
...
export default async function ImportarContasAPagarPage({
  ...
  const podeLancar = hasPermission(session.membership.role, "receivable.create");
  ...
  return (
    <div>
      <PageHeader
        title="Importar títulos a receber"
      ...
      {!podeLancar ? (
        ...
        <Card title="Importação concluída">
          ...
          <div className="mt-4 flex gap-3">
            ...
            <Link href="/contas-a-receber/importar" className="text-sm text-[var(--brand)] underline">
        ...
        <>
          <Card className="mb-4" title="Resumo da conferência">
            ...
            <form action={confirmarImportacaoAction} className="mt-4 flex flex-wrap items-center gap-3">
              ...
              <Link href="/contas-a-receber/importar" className="text-sm text-[var(--ink-muted)] underline">
                Cancelar
              </Link>
            </form>
          </Card>
          ...
          {previa.erros.length > 0 ? (
            <Card title={`Linhas com erro (${previa.erros.length})`}>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-3">
                <a
                  href={`/contas-a-receber/importar/erros?previa=${sp.previa}`}
                  className="text-sm text-[var(--brand)] underline"
                >
        ...
        <Card title="1 — Envie o arquivo">
          <p className="mb-3 text-sm text-[var(--ink-muted)]">
            O arquivo deve ter as colunas: {IMPORT_CSV_COLUMNS.join("; ")}. Cliente, categoria e
            centro de custo precisam existir nos cadastros.
          </p>
          <a
            href="/contas-a-receber/importar/modelo"
