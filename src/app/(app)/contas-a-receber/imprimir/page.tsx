...
/**
 * Visão de impressão de Contas a Receber: só o cabeçalho, a tabela filtrada e os
...
export default async function ImprimirContasAPagarPage({
  ...
  const dados = await loadFilteredReceivables(repos, session.company.id, filtros);
  ...
  return (
    <div className="print-root">
      ...
      <header className="mb-4">
        ...
        <p className="text-lg">Contas a Receber</p>
        <p className="text-xs text-[var(--ink-muted)]">
          {describeFilters(filtros, dados.customerNameFiltrado)}
        </p>
      ...
      <table className="w-full border-collapse text-[11px]">
        <thead>
          <tr>
            {RECEIVABLE_EXPORT_COLUMNS.map((c) => (
              <th
                key={c}
              ...
              {RECEIVABLE_EXPORT_COLUMNS.map((c) => (
                <td
                  ...
                  className={`px-1.5 py-1 align-top ${
                    c === "Valor (R$)" || c === "Valor Recebido (R$)" ? "text-right tabular" : ""
                  }`}
                >
                  {r[c]}
                </td>
        ...
        <tfoot>
          <tr className="font-semibold">
            ...
            <td className="border-t border-[#999] px-1.5 py-1 text-right tabular">
              {formatBRL(totais.recebidoCents)}
            </td>
