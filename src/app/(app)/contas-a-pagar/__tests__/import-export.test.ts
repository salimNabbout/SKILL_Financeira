...
describe("filtros da listagem", () => {
  it("período explícito vence Ano/Mês", () => {
    const f = parsePayableFilters({ ano: "2026", mes: "3", de: "2026-08-01", ate: "2026-08-31" }, HOJE);
    expect(f.dueFrom).toBe("2026-08-01");
    expect(f.dueTo).toBe("2026-08-31");
  });

  it("ano sem mês cobre o ano inteiro; com mês, o mês fechado", () => {
    expect(parsePayableFilters({ ano: "2026" }, HOJE).dueFrom).toBe("2026-01-01");
    expect(parsePayableFilters({ ano: "2026" }, HOJE).dueTo).toBe("2026-12-31");
    expect(parsePayableFilters({ ano: "2026", mes: "2" }, HOJE).dueTo).toBe("2026-02-28");
  });

  it("acusa período invertido em vez de devolver lista vazia", () => {
    expect(parsePayableFilters({ de: "2026-08-31", ate: "2026-08-01" }, HOJE).periodoInvalido).toBe(true);
  });

  it("situação inválida cai para todos, sem filtrar nada fora", () => {
    const f = parsePayableFilters({ status: "inventado" }, HOJE);
    expect(f.situacao).toBe("todos");
    expect(f.statuses).toBeUndefined();
  });

  it("Atrasado recorta os títulos em aberto vencidos até ontem", () => {
    const f = parsePayableFilters({ status: "atrasado" }, HOJE);
    expect(f.statuses).toEqual(["open", "scheduled", "partially_paid"]);
    expect(f.dueTo).toBe("2026-08-19");
    expect(f.dueFrom).toBeUndefined();
  });

  it("Hoje recorta exatamente o dia", () => {
    const f = parsePayableFilters({ status: "hoje" }, HOJE);
    expect(f.dueFrom).toBe(HOJE);
    expect(f.dueTo).toBe(HOJE);
  });

  it("situação e período se combinam por interseção, sem um anular o outro", () => {
    const f = parsePayableFilters({ status: "a_vencer", de: "2026-08-01", ate: "2026-12-31" }, HOJE);
    expect(f.dueFrom).toBe("2026-08-21");
    expect(f.dueTo).toBe("2026-12-31");
  });

  it("a query preserva os filtros para export e impressão", () => {
    const q = filtersToQuery({ status: "atrasado", ano: "2026", fornecedor: "sup_1" });
  ...
  it("descreve os filtros para o cabeçalho do relatório", () => {
    const f = parsePayableFilters({ status: "pago", ano: "2026", mes: "5" }, HOJE);
    expect(describeFilters(f, "Alfa")).toContain("Situação: Pago");
